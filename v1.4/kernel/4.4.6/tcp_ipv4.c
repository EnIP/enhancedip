...
int tcp_save_extended_ip(struct sock *sk, struct sk_buff *skb)
{
       ...
       if(th->syn == 1 && th->ack == 0){
               printk("tcp_save_extended_ip() - SYN packet being received\n");
               if(ip->ihl == 8){
                  eip = (struct extended_ip *)(ip + 1);
 
                  if(eip->optionid == 0x9a){
                       sk->ext_daddr = ntohl(eip->extended_saddr);
                       printk("tcp_save_extended_ip() - extended daddr = 0x%x\n", sk->ext_daddr);
                  }
		  else{
                       printk("tcp_save_extended_ip() - eip->optionid != 0x9a\n");
		  }
                  
               }
	       else{
		   printk("tcp_save_extended_ip() - ip->ihl != 8\n");
	       }	
       }
...
static void tcp_v4_send_reset(const struct sock *sk, struct sk_buff *skb)
{
	...
	printk("EXPERIMENT1: tcp_v4_send_reset()\n");
