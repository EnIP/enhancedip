...
int main(int argc, char* argv[])
{
	...
	if(with_options)
        	retval = trace_tcp_options(dst_addr, src_addr, extended_src, extended_dst, &replyip, x, 40508, tcpport, "EnIP", 4, &difference);
	else
        	retval = trace_tcp(dst_addr, src_addr, &replyip, x, 40508, tcpport, "EnIP", 4, &difference);

        if(retval < 0){
                printf("fatal error on ttl %d, error=%d\n", x, retval);
		exit(EXIT_FAILURE);
        }
        else if(retval == 100){
                printf(".\n");
