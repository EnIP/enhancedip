...
#include <linux/eip.h>  //enhanced ip
...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	int retval = 0;
        struct extended_ip eip;	//enhanced ip
	...
	printk("UDP: sk->sk_socket=0x%x, sk->ext_daddr=0x%x\n", sk->sk_socket, sk->ext_daddr);
...
/**
 * enhanced ip (enip)
 * if we are receiving a udp packet that contains an enhanced
		   ...
		   int proto)
{
	...
	if (sk) {
		...
		udp_save_extended_ip(sk, skb); //enhanced ip
